function standart_transferctrl($scope,$rootScope,$window,srv,$route)
{
    function InitGrid()
    {
        $("#TblGrid").dxDataGrid
    ...
    $scope.BtnTransfer = async function()
    {
        ...
        for (let i = 0; i < window.Schema.length; i++) 
        {
            ...
            {   
                ...
                srv.Alert('Aktarım Tamamlandı',$scope.InsertInfo.length + ' Adet Yeni Kayıt <br>' + $scope.UpdateInfo.length + ' Adet Güncelleme <br>' + $scope.ErrorGrid.length + ' Adet Başarısız <br>' + 'Aktarım Tamamlandı.','success');
                Clear();
            }
        }
    }
